...
export default function App() {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'terms', 'privacy', 'vendor'
  ...
  const colors = {
    ...
    accent: `text-[#A54600]`, import React, { useState, useEffect } from 'react';
import { 
  Menu, 
  X, 
  ShieldCheck, 
...
const SecretUploadForm = () => {
  ...
  const handleUpload = async (e) => {
    ...
    // NOTE: Replace this with your actual Firebase/Backend API call
    setTimeout(() => {
        ...
        <div className="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              ...
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#A54600] outline-none"
            />
        ...
        <form onSubmit={handleUpload} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <input required type="text" value={merchantId} onChange={(e) => setMerchantId(e.target.value)} placeholder="e.g. mer_888999" className="w-full px-4 py-3 border border-slate-300 rounded-lg outline-none focus:border-[#A54600]"/>
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Price (₦)</label>
              <input required type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g. 150000" className="w-full px-4 py-3 border border-slate-300 rounded-lg outline-none focus:border-[#A54600]"/>
            </div>
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Product Name</label>
            <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} placeholder="e.g. iPhone 13 Pro Max" className="w-full px-4 py-3 border border-slate-300 rounded-lg outline-none focus:border-[#A54600]"/>
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Direct Image URL</label>
            <input required type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="https://..." className="w-full px-4 py-3 border border-slate-300 rounded-lg outline-none focus:border-[#A54600]"/>
          </div>
          <button disabled={loading} type="submit" className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800">
            <Plus size={20} /> {loading ? 'Injecting...' : 'Inject Product to Database'}
          </button>
        </form>
      </div>
    </div>
...
export default function App() {
  ...
  // Simple router logic: If URL has ?admin=true, show the Secret Upload Form
  useEffect(() => {
  ...
  const faqs = [
    {
      ...
      answer: (
        <div className="space-y-3">
          ...
          <p>To maintain the integrity of the reservation system, once funds are committed, they stay within the system. If you cancel, your balance is immediately converted into <strong>Store Credit</strong>.</p>
        </div>
      )
    },
    {
      ...
      answer: (
        <div className="space-y-3">
          <p><strong>Store Credit</strong> works like a digital wallet balance for a specific Merchant.</p>
          <p>For example: If you cancel an iPhone purchase, that money sits in your "Store Credit" wallet with that specific Merchant. You can use it to buy a Samsung, AirPods, or any other item from them immediately. You cannot move the credit to a different Merchant's store.</p>
        </div>
      )
    },
    {
      question: "What happens if I miss a payment?",
      answer: "We understand things happen. You have a grace period to make your payment. If you completely default and the time expires, the plan will be automatically cancelled, and your money will be safely converted into Store Credit for that Merchant."
    },
    {
      question: "How do I find a Merchant?",
      answer: "You can walk directly into any physical store that has the 'Korra Accepted Here' sticker, or you can browse our online directory to find trusted Merchants near you. You negotiate the price with them, and they will give you a Payment Code to enter in the app."
    },
    {
      question: "Why isn't the app on the Play Store yet?",
      answer: "We are currently in a Direct Release phase for early adopters in select cities. This allows us to push high-frequency updates without waiting for third-party approvals."
    },
    {
  ...
  return (
    <div className={`min-h-screen bg-white text-slate-900 font-sans selection:bg-[#A54600] selection:text-white overflow-x-hidden`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 md:h-20">
            ...
            <div className="flex-shrink-0 flex items-center gap-2 cursor-pointer">
              <img src="/korra_logo_icon.webp" alt="Korra Logo" className="h-10 w-10 md:h-12 md:w-12 object-contain" />
              <span className="font-bold text-xl md:text-2xl tracking-tight text-slate-900">Korra</span>
            </div>

            <div className="hidden md:flex items-center space-x-8">
              <a href="#models" className="hover:text-[#A54600] transition-colors text-sm font-medium">Models</a>
              <a href="#benefits" className="hover:text-[#A54600] transition-colors text-sm font-medium">Benefits</a>
              <a href="#about" className="hover:text-[#A54600] transition-colors text-sm font-medium">About</a>
              <a href="#faq" className="hover:text-[#A54600] transition-colors text-sm font-medium">FAQs</a>
              <button onClick={scrollToDownload} className="bg-[#A54600] text-white px-6 py-2.5 rounded-full font-bold text-sm transition-all transform hover:scale-105 shadow-xl shadow-[#A54600]/20">
                Get The App
              </button>
            </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-b border-slate-200 absolute w-full left-0 top-16 shadow-lg">
            <div className="px-4 pt-4 pb-6 space-y-3">
              ...
              <a href="#about" onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 rounded-md text-base font-medium hover:bg-slate-50">About</a>
              ...
              <div className="pt-2">
                <button onClick={scrollToDownload} className="w-full bg-[#A54600] text-white px-4 py-3.5 rounded-xl font-bold text-base shadow-md">
                  Get The App
                </button>
              </div>
            </div>
      ...
      {/* --- HERO SECTION --- */}
      <section className="relative pt-28 pb-16 lg:pt-40 lg:pb-32 overflow-hidden bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm mb-6 md:mb-8 mx-auto">
      ...
      <section id="download-section" className="py-16 md:py-20 bg-white border-y border-slate-100">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-2xl md:text-4xl font-bold mb-4 text-slate-900">Get Started Today</h2>
            <p className="text-slate-600 max-w-2xl mx-auto text-sm md:text-base mb-10">
              Download the right app for your needs. Available via direct APK or universally through our Web App.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              ...
              <div className="bg-slate-50 rounded-3xl p-8 border border-slate-200 flex flex-col items-center">
                <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center mb-6">
                  <ShieldCheck className="w-8 h-8 text-[#A54600]" />
                </div>
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Korra</h3>
                <p className="text-slate-500 text-sm mb-8">For Customers: Lock prices and pay small-small.</p>
                
                <div className="w-full space-y-3">
                  {/* TODO: UPDATE THIS HREF WITH YOUR REAL CUSTOMER APK LINK */}
                  <a href="https://app.korra.com.ng/downloads/korra.apk" className="w-full flex items-center justify-center gap-3 bg-slate-900 text-white px-6 py-4 rounded-xl hover:bg-slate-800 transition-all font-bold">
                    <Smartphone className="w-5 h-5" /> Download APK (Android)
                  </a>
                  
                  {/* TODO: UPDATE THIS HREF WITH YOUR REAL CUSTOMER WEB APP LINK */}
                  <a href="https://app.korra.com.ng" target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-3 bg-white text-slate-900 px-6 py-4 rounded-xl border border-slate-200 hover:bg-slate-50 transition-all font-bold">
                    <Globe className="w-5 h-5 text-[#A54600]" /> Use Web App (iOS/Any)
                  </a>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-3xl p-8 border border-slate-200 flex flex-col items-center">
                ...
                <p className="text-slate-500 text-sm mb-8">For Merchants: Manage reservations and get paid.</p>
                
                <div className="w-full space-y-3">
                  {/* TODO: UPDATE THIS HREF WITH YOUR REAL BUSINESS APK LINK */}
                  ...
                  {/* TODO: UPDATE THIS HREF WITH YOUR REAL BUSINESS WEB APP LINK */}
                  <a href="https://business.korra.com.ng" target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-3 bg-white text-slate-900 px-6 py-4 rounded-xl border border-slate-200 hover:bg-slate-50 transition-all font-bold">
                    <Globe className="w-5 h-5 text-slate-900" /> Use Web Portal (PC/iOS)
                  </a>
                </div>
              </div>
            ...
            <p className="text-xs text-slate-400 mt-8">
              iOS users or Android users facing download issues can use the "Use Web App" buttons above. Simply tap 'Share' in your browser and select 'Add to Home Screen' for the full app experience.
            </p>
         </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6 lg:gap-12">
            ...
            <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm">
              ...
              <ul className="space-y-5">
                ...
                <li className="flex gap-4">
                  <div className="mt-1 shrink-0"><Lock className="text-slate-900 w-5 h-5" /></div>
                  <div>
                    <h4 className="font-bold text-slate-900 text-sm md:text-base">Active Plan Limits (Caps)</h4>
                    <p className="text-xs md:text-sm text-slate-600 mt-1">
                      New users can have up to 3 active plans at a time. Complete plans successfully to level up and unlock more slots.
                    </p>
                  </div>
                </li>
                <li className="flex gap-4">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <div className="grid md:grid-cols-2 gap-12">
              <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 text-center">
                 <h3 className="font-bold text-xl mb-4 text-[#A54600]">Merchants</h3>
                 <p className="text-slate-600">Stop using notebooks to track who owes you what. Korra automates the entire collection process, filters out unserious buyers, and gives you instant cash flow.</p>
              </div>
              <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 text-center">
                 <h3 className="font-bold text-xl mb-4 text-slate-900">Customers</h3>
                 <p className="text-slate-600">Don't let inflation eat your savings. The price is locked the moment you pay your deposit. No price hikes. No awkward conversations. We handle the schedule.</p>
              </div>
           </div>
        </div>
      </section>
      ...
      {/* --- ABOUT US SECTION --- */}
      <section id="about" className="py-16 md:py-20 bg-white border-b border-slate-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-slate-600 text-lg leading-relaxed mb-8">
            We are not a marketplace. We do not list products. We provide the digital agreement and payment rails for Merchants and Customers to transact securely over time.
          </p>
        </div>
      </section>
          ...
          {sections.map((section, idx) => (
            <div key={idx}>
              <h3 className="text-base font-bold text-slate-900 mb-3 flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#A54600]"></span>{section.heading}
              </h3>
...
const merchantAgreementData = [
  ...
  { heading: '2. Fees & Commission', items: ['Korra charges a flat 3.5% Platform Fee on transactions.', 'This fee covers automated bookkeeping and payment infrastructure.'] },
  { heading: '3. No Refunds Policy', items: ['All transactions are considered final once settled to your wallet.', 'Korra does not facilitate cash reversals for "change of mind". Any cancellations are processed strictly as Store Credit to be spent in your store.'] }
];
...
const customerTermsData = [
  ...
  { heading: '3. Commitment Policy (No Refunds)', items: ['By initiating a plan, you are committing to the purchase.', 'We do not offer cash refunds. If you cancel a plan or default on payment, your balance is converted to Store Credit valid only with that specific Merchant.'] }
];

const privacyData = [
  { heading: '1. Data Collection', items: ['We collect personal details and transaction history to provide our service.', 'We comply with all relevant data protection regulations in Nigeria.'] },
  { heading: '2. Security', items: ['We employ bank-grade encryption to protect your data.', 'Your financial data is processed securely by regulated partners like Monnify.'] }
];
  ...
  // --- UPDATED FAQS (PROFESSIONAL TONE + STORE CREDIT EXPLANATION) ---
  const faqs = [
    {
      question: "Can I cancel my plan? (Refund Policy)",
      answer: (
    ...
    },
    {
      ...
      answer: (
        <div className="space-y-3">
          <p><strong>Store Credit</strong> works like a digital wallet balance for a specific Vendor. Your money is never lost—it is simply re-assigned.</p>
          <p>For example: If you cancel an iPhone purchase, that money sits in your "Store Credit" wallet with that vendor. You can use it to buy a Samsung, AirPods, or any other item from them in the future.</p>
        </div>
      )
    },
    {
      question: "Why isn't the app on the Play Store yet?",
      answer: "We are currently in a Direct Release phase for early adopters. This allows us to push high-frequency updates without waiting for third-party approvals. You get the latest features days before the general public."
    },
    {
      question: "What is the 3.5% Initiation Fee?",
      answer: "This is a one-time architecture fee. It covers the payment rails, the automated bookkeeping technology, and the security protocols that hold the deal together. It is non-refundable."
    },
    {
    ...
    <div className={`min-h-screen ${colors.primaryBg} ${colors.text} font-sans selection:bg-[#A54600] selection:text-white overflow-x-hidden`}>
      ...
      <section id="download-section" className="relative pt-28 pb-16 lg:pt-48 lg:pb-32 overflow-hidden bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          {/* DOWNLOAD BUTTONS (NOW WITH REAL LINKS) */}
          <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-4 items-stretch sm:items-center px-4 md:px-0">
            ...
            <a 
              ...
              className="flex items-center gap-3 bg-slate-900 text-white px-6 py-4 rounded-xl hover:bg-slate-800 transition-all shadow-lg active:scale-95 justify-center border border-slate-900"
            >
            ...
            <a 
              ...
              className="flex items-center gap-3 bg-white text-slate-900 px-6 py-4 rounded-xl hover:bg-slate-50 transition-all shadow-lg active:scale-95 justify-center border border-slate-200"
            >
