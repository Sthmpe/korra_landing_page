...
import { useLiveMerchants } from '../../lib/useLiveMerchants';

const slugify = (name = '') =>
  name.toLowerCase().trim().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');

export default function MerchantsDirectoryClient() {
  const { merchantsList: liveMerchants, loading } = useLiveMerchants();
  ...
  return (
    <div className="page">
          ...
          {loading ? (
            ...
            <div className="directory-grid stagger reveal">
              {filtered.map((merchant) => {
                ...
                return (
                  <Link href={`/merchant/${slug}`} key={merchant.id} className="merchant-card">
                    ...
                    <div className="merchant-socials">
                      <div className="merchant-social-icons">
                        {merchant.socials?.whatsapp && (
                          <a href={merchant.socials.whatsapp} onClick={(e) => e.stopPropagation()} target="_blank" rel="noreferrer" className="social-btn wa" aria-label="WhatsApp"><WhatsAppIcon className="ic" /></a>
                        )}
                        {merchant.socials?.instagram && (
                          <a href={merchant.socials.instagram} onClick={(e) => e.stopPropagation()} target="_blank" rel="noreferrer" className="social-btn ig" aria-label="Instagram"><InstagramIcon className="ic" /></a>
                        )}
                        {merchant.socials?.tiktok && (
                          <a href={merchant.socials.tiktok} onClick={(e) => e.stopPropagation()} target="_blank" rel="noreferrer" className="social-btn tt" aria-label="TikTok"><TikTokIcon className="ic" /></a>
                        )}
                        {merchant.socials?.website && (
                          <a href={merchant.socials.website} onClick={(e) => e.stopPropagation()} target="_blank" rel="noreferrer" className="social-btn web" aria-label="Website"><Globe className="ic" /></a>
                        )}
                      </div>
      ...
      <style>{`
        ...
        .merchant-socials { display: flex; align-items: center; justify-content: space-between; gap: 10px; }
        .merchant-social-icons { display: flex; gap: 7px; }
        ...
        .merchant-view {
          ...
          display: inline-flex; align-items: center; gap: 5px; transition: color .2s var(--ease); white-space: nowrap;
        }
