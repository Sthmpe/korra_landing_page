...
// Legacy slug (old links used only space->dash). Kept as a fallback so any
// previously-shared URL still resolves.
const legacySlug = (name = '') => name.toLowerCase().replace(/\s+/g, '-');

export default function MerchantProfileClient() {
  const { slug } = useParams();
  const { merchantsList: liveMerchants, loading } = useLiveMerchants();
  const merchant = liveMerchants.find(
    (m) => slugify(m.name) === slug || legacySlug(m.name) === slug
  );
    ...
    <div className="page">
      ...
      <section className="section profile-page">
        <div className="container">
          <Link href="/merchants" className="profile-back">
          ...
          {!loading && !merchant && (
            <div className="profile-state">
              ...
              <Link href="/merchants" className="btn btn-primary">Back to directory <span className="arrow">→</span></Link>
            </div>
          )}
